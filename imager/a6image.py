...
class Image(object):
    ...
    def getData(self):
        ...
        """
        image_copy = self._data[:]  
        if _is_pixel_list(self._data): 
            return image_copy
    ...
    def setWidth(self,value):
        ...
        assert (value > 0), repr(value) + ' is not greater than 0'  # I need to reask if i should make it >=0 or just >
        assert (len(self._data) % value == 0), repr(value) + ' is an invalid value'
    ...
    def setHeight(self,value):
        ...
        """
        assert (type(value) == int), repr(value) + ' is not an integer'
        assert (value > 0), repr(value) + ' is less than 0'  # I need to reask if i should make it >=0 or just >
        assert (len(self._data) % value == 0), repr(value) + ' is an invalid value'
    ...
    def __init__(self, data, width):
        ...
        """
        assert _is_pixel_list(data), repr(data) + ' is an invalid pixel list'
        assert (type(width) == int), repr(width) + ' is not an integer'
        assert (width > 0), repr(width) + ' is an invalid width'
        assert (len(data) % width == 0), repr(width) + ' is an invalid width'

        self._width = width

        if _is_pixel_list(data):
            self._data = data  #should i slice or just use assignment statement?
    ...
    def __getitem__(self, pos):
        ...
        """
        assert type(pos) == int, repr(pos) + ' is not an integer'
        assert pos >= 0, repr(pos) + ' is less than 0'
        assert pos in range(len(self._data)), repr(pos) + ' is out of range'
    ...
    def __setitem__(self, pos, pixel):
        ...
        assert type(pos) == int, repr(pos) + ' is not an integer'
        assert pos >= 0, repr(pos) + ' is less than 0'
        assert _is_pixel(pixel), repr(pixel) + ' is invalid pixel'
        assert pos in range(len(self._data)), repr(pos) + ' is out of range'
    ...
    def getPixel(self, row, col):
        ...
        """
        assert type(row) == int, repr(row) + ' is not an integer'
        assert (row >= 0 and row < self._height), repr(row)+' is out of range'
        assert type(col) == int, repr(col) + ' is not an integer'
        assert (col >= 0 and col < self._width), repr(col)+' is out of range'
    ...
    def __str__(self):
        ...
        for r in range(self._height):
            added_row = []
            for c in range(self._width*r, self._width*r + self._width ):
                added_row.append(self._data[c])
            result.append(str(added_row))
